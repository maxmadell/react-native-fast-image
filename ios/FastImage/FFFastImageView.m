...
- (void)setSource:(FFFastImageSource *)source {
    if (_source != source) {
        ...
        if (!CGSizeEqualToSize(_source.resize, CGSizeZero)) {
            context = @{SDWebImageContextImageTransformer: [SDImage transformerWithSize:_source.resize
		...
		[self sd_setImageWithURL:<#(nullable NSURL *)#> placeholderImage:<#(nullable UIImage *)#> options:<#(SDWebImageOptions)#> progress:<#^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL)progressBlock#> completed:<#^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL)completedBlock#>]
